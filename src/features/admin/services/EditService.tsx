...
function getCategoryLabel(category: string): string {
    return `Sobre o Serviço (${category})`;
}
...
export default function EditService() {
    ...
    const [formData, setFormData] = useState<any>({
        name: "", image: "", category: "Agência", highlight: false, distance: "Naviraí - MS", latitude: 0, longitude: 0,
        features: [""],
        about: { title: "Sobre o Serviço", subtitle: "Qualidade e Confiança", desc: [""] },
        accommodation: { title: "Estrutura / Frota / Detalhes", image: "", imageCaption: "Foto Geral", desc: [""] },
        policies: [],
        amenities: { title: "Diferenciais e Facilidades", cards: [] },
        gallery: [],
        cta: { title: "Deseja contratar ou fazer um orçamento?", desc: "Fale conosco agora mesmo para tirar suas dúvidas!" }
    });
    ...
    useEffect(() => {
        const fetchServiceData = async () => {
            if (!id) return;
            try {
                const res = await apiFetch(`${API_BASE}/api/services/${id}`);
                if (res.ok) {
    ...
    const SCHEDULE_PRESETS = [
        { label: "08h-18h", title: "Atendimento Comercial", desc: "Segunda a Sexta, das 08h às 18h" },
        { label: "24h", title: "Plantão Permanente", desc: "Disponível 24h por dia para emergências" },
        { label: "Sab-Dom", title: "Finais de Semana", desc: "Sábado e Domingo, das 09h às 13h" }
    ];
    ...
    const handleAddAmenity = () => {
        setFormData({ ...formData, amenities: { ...formData.amenities, cards: [...(formData.amenities?.cards || []), { title: "Diferencial", desc: "Breve explicação sobre este diferencial" }] } });
    };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const response = await apiFetch(`${API_BASE}/api/services/${id}`, {
                method: "PUT",
    ...
    return (
        <div className="max-w-5xl mx-auto space-y-6">
            <button onClick={() => navigate("/admin/servicos")} className="flex items-center gap-2 text-[#8a7968] dark:text-[#c5b49e] hover:text-[#241a06] dark:hover:text-white transition-colors font-medium cursor-pointer">
            ...
            <div className="bg-white dark:bg-[#241a06] rounded-3xl p-6 md:p-10 shadow-sm border border-[#ede0d8] dark:border-[#3a2e1a]">
                <div className="flex items-center gap-4 mb-2">
                    ...
                    <div>
                        ...
                        <p className="text-[#8a7968] dark:text-[#c5b49e]">Atualizando os dados do serviço "{formData.name}".</p>
                    </div>
                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-12">
                    ...
                    <section>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div className="space-y-2">
                                <label className="block text-sm font-bold text-[#241a06] dark:text-[#f0e6d6] ml-1">Nome do Serviço</label>
                            ...
                            <div className="space-y-2">
                                <label className="block text-sm font-bold text-[#241a06] dark:text-[#f0e6d6] ml-1">Tipo de Serviço</label>
                                <select className="w-full px-5 py-4 bg-[#faf5f0] dark:bg-[#1a1208] border border-[#ede0d8] dark:border-[#3a2e1a] text-[#241a06] dark:text-[#f0e6d6] rounded-2xl focus:ring-2 focus:ring-(--color-primary) outline-none transition-all shadow-xs appearance-none font-medium cursor-pointer"
                                    value={formData.category || "Agência"} onChange={(e) => setFormData({ ...formData, category: e.target.value })}>
                                    <option value="Agência">Agência de Turismo</option>
                                    <option value="Espaços para Eventos">Espaço para Eventos</option>
                                    <option value="Locadora de Veículos">Locadora de Veículos</option>
                                    <option value="Organizadoras de Eventos">Organizadora de Eventos</option>
                                    <option value="Taxi">Táxi / Transporte Particular</option>
                                    <option value="Transportadoras Rodoviárias">Transportadora Rodoviária</option>
                                    <option value="Transportadoras Turísticas">Transportadora Turística</option>
                                </select>
                            </div>
                        </div>
                    </section>
                    ...
                    <section>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            {/* Foto da Estrutura */}
                            <div className="space-y-2">
                                <label className="block text-sm font-bold text-[#241a06] dark:text-[#f0e6d6] ml-1">Imagem da Estrutura / Veículo</label>
                                {formData.accommodation?.image && !newAccommodationFile && (
                                    <div className="w-full h-32 rounded-2xl overflow-hidden border border-[#ede0d8] dark:border-[#3a2e1a] mb-2">
                                        <img src={formData.accommodation.image} alt="Estrutura atual" className="w-full h-full object-cover" />
                                    </div>
                                )}
                                {newAccommodationFile && (
                                    <div className="w-full h-32 rounded-2xl overflow-hidden border border-green-300 dark:border-green-600 mb-2 relative group">
                                        <img src={URL.createObjectURL(newAccommodationFile)} alt="Nova estrutura" className="w-full h-full object-cover" />
                                ...
                                >
                                    ...
                                    <span className="text-[#8a7968] dark:text-[#c5b49e] text-sm font-medium text-center">
                                        {dragStates.accommodation ? '📸 Solte aqui!' : (newAccommodationFile ? newAccommodationFile.name : "Clique ou arraste para trocar foto complementar")}
                                    </span>
                    ...
                    <section className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div className="space-y-4">
                            ...
                            <textarea required rows={6} className="w-full px-5 py-4 bg-[#faf5f0] dark:bg-[#1a1208] border border-[#ede0d8] dark:border-[#3a2e1a] text-[#241a06] dark:text-[#f0e6d6] placeholder:text-[#8a7968] dark:placeholder:text-[#c5b49e]/50 rounded-2xl focus:ring-2 focus:ring-(--color-primary) outline-none transition-all shadow-xs resize-none"
                                placeholder="Descrição detalhada sobre o funcionamento e o que consiste o serviço..."
                        ...
                        <div className="space-y-4">
                            <h3 className="text-lg font-bold text-[#241a06] dark:text-[#f0e6d6] flex items-center gap-2">
                                ...
                                Detalhes de Estrutura / Frota
                            </h3>
                            <textarea required rows={6} className="w-full px-5 py-4 bg-[#faf5f0] dark:bg-[#1a1208] border border-[#ede0d8] dark:border-[#3a2e1a] text-[#241a06] dark:text-[#f0e6d6] placeholder:text-[#8a7968] dark:placeholder:text-[#c5b49e]/50 rounded-2xl focus:ring-2 focus:ring-(--color-primary) outline-none transition-all shadow-xs resize-none"
                                placeholder="Informações sobre veículos, dependências físicas, regras de locação, etc..."
                                ...
                                return (
                                    <div key={idx} className={`border p-6 rounded-2xl relative group transition-all hover:shadow-md ${isSchedule ? 'bg-emerald-50/50 dark:bg-emerald-950/20 border-emerald-200 dark:border-emerald-900/40 hover:bg-white dark:hover:bg-[#2e2310]/50' : 'bg-amber-50/50 dark:bg-amber-950/20 border-amber-200 dark:border-amber-900/40 hover:bg-white dark:hover:bg-[#2e2310]/50'}`}>
                                        <div className="flex items-center justify-between mb-3">
                                            ...
                                            <button type="button" onClick={() => handleRemovePolicy(idx)} className="text-[#8a7968] bg-red-200 dark:bg-red-950/60 dark:text-red-400 rounded-full p-2 hover:text-red-500 hover:bg-red-300 dark:hover:bg-red-900/60 transition-colors cursor-pointer">
                                                <Trash2 size={16} />
                                            </button>
                                        </div>
                                        <div className="space-y-3">
                                            {isSchedule && (
                                                <input type="text" placeholder="Horário (ex: 08h-18h)"
                                            ...
                                            <input type="text" placeholder={isSchedule ? 'Título (ex: Atendimento)' : 'Título da Regra (ex: Reservas)'}
                                            ...
                                            <textarea placeholder={isSchedule ? 'Descrição' : 'Detalhes da regra'}
                    ...
                    {/* ═══ BLOCO 5: Comodidades / Diferenciais ═══ */}
                    <section>
                        <div className="flex items-center justify-between mb-6">
                            <h3 className="text-xl font-bold text-[#241a06] dark:text-[#f0e6d6] flex items-center gap-2">
                                ...
                                Diferenciais e Facilidades (Cards)
                            </h3>
                            <button type="button" onClick={handleAddAmenity} className="flex items-center gap-2 text-sm font-bold text-(--color-primary) bg-(--color-primary)/10 px-4 py-2 rounded-xl hover:bg-(--color-primary)/20 transition-all border border-(--color-primary)/20 cursor-pointer">
                                <Plus size={18} /> Adicionar Diferencial
                            </button>
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            {formData.amenities?.cards?.map((card: any, idx: number) => (
                                <div key={idx} className="bg-[#faf5f0] dark:bg-[#1a1208]/60 border border-[#ede0d8] dark:border-[#3a2e1a] p-6 rounded-2xl flex gap-6 items-start relative group hover:bg-white dark:hover:bg-[#241a06] hover:shadow-md transition-all">
                                    ...
                                    <button type="button" onClick={() => { const nc = [...formData.amenities.cards]; nc.splice(idx, 1); setFormData({ ...formData, amenities: { ...formData.amenities, cards: nc } }); }} className="p-3 text-[#8a7968] bg-red-200 dark:bg-red-950/60 dark:text-red-400 rounded-xl hover:text-red-500 hover:bg-red-300 dark:hover:bg-red-900/60 transition-all cursor-pointer">
                                        <Trash2 size={22} />
                                    </button>
                                </div>
                            ))}
