...
const CATEGORY_FILTERS = [
    ...
    { label: "Basquete", value: "Basket" },
    { label: "Vôlei", value: "Vôlei" },
    { label: "Beisebol", value: "Beisebol" },
];

export default function ListSports() {
    const [sportsData, setSportsData] = useState<Venue[]>([]);
    ...
    const fetchSports = async () => {
        ...
        try {
            ...
            console.error("Failed to fetch sports from DB.");
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleDelete = async () => {
        ...
        try {
            const token = localStorage.getItem("token");
            const res = await apiFetch(`${API_BASE_URL}/api/sports/${sportToDelete}`, {
                method: "DELETE",
                headers: { "Authorization": `Bearer ${token}` }
            });
            if (res.ok) {
                setSportsData(sportsData.filter(s => s._id !== sportToDelete));
                setShowDeleteModal(false);
                setSportToDelete(null);
                toast.success("Local esportivo excluído com sucesso!");
            } else {
                toast.error("Erro ao excluir local esportivo.");
            }
        } catch (error) {
            console.error("Error deleting sport venue:", error);
            toast.error("Erro ao excluir local esportivo.");
        }
    };
    ...
    const countByCategory = (cat: string) => {
        if (cat === "all") return sportsData.length;
        return sportsData.filter(s => (s as any).category === cat).length;
    };
